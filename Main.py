...
def app():
    ...
    # --- HEADER ---
    st.markdown("<div class='main-title'>AI vs. Fact: An NLP Model Comparison</div>", unsafe_allow_html=True)
    st.markdown("<div class='sub-title'>A comparative analysis of linguistic features in political fact-checking</div>", unsafe_allow_html=True)
    ...
    # ============================
    with col_left:
        st.markdown("<div class='section-title'>1. Data Collection</div>", unsafe_allow_html=True)
        st.caption("Scrape verified political statements from PolitiFact.")
        ...
        if st.button("Scrape Data"):
            if start_date > end_date:
                ...
                with st.spinner("Scraping factual statements..."):
                    scraped_df = scrape_data_by_date_range(pd.to_datetime(start_date), pd.to_datetime(end_date))
                if not scraped_df.empty:
                    st.session_state['scraped_df'] = scraped_df
                    st.success(f"Data collected: {len(scraped_df)} statements.")
                else:
                    st.warning("No data retrieved. Adjust date range.")

        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown("<div class='section-title'>2. Analysis Configuration</div>", unsafe_allow_html=True)
        ...
        selected_phase = st.selectbox("Select NLP Feature Phase", phases, key='selected_phase')
    ...
    # ============================
    with col_center:
        st.markdown("<div class='section-title'>3. Model Performance Summary</div>", unsafe_allow_html=True)
        
        if st.session_state['df_results'].empty:
            st.info("Results will appear here after analysis.")
        else:
            df_results = st.session_state['df_results']
            st.markdown(f"**Feature Phase:** {st.session_state['selected_phase_run']}")
            st.dataframe(
            ...
            plot_metric = st.selectbox("Select Metric", metrics, index=1, key='plot_metric_center')

            df_plot = df_results[['Model', plot_metric]].set_index('Model')
            st.bar_chart(df_plot)

            st.caption(f"Average scores over {N_SPLITS}-fold cross-validation for the {st.session_state['selected_phase_run']} features.")

    # ============================
    # RIGHT COLUMN: Critique
    # ============================
    with col_right:
        st.markdown("<div class='section-title'>4. Discussion & Insights</div>", unsafe_allow_html=True)
        
        if st.session_state['df_results'].empty:
            st.info("Insights will be generated after training.")
        else:
            ...
            x_axis = st.selectbox("X-Axis (Speed)", metrics_speed, key='x_axis', index=1)
            y_axis = st.selectbox("Y-Axis (Quality)", metrics_quality, key='y_axis', index=0)

            fig, ax = plt.subplots(figsize=(6, 4))
            df_results = st.session_state['df_results']

            ax.scatter(df_results[x_axis], df_results[y_axis], s=120, color="#1a365d", alpha=0.75)
            for i, row in df_results.iterrows():
                ax.annotate(row['Model'], (row[x_axis]*1.02, row[y_axis]*0.98), fontsize=9, color="#2d3748")

            ax.set_xlabel(x_axis)
            ax.set_ylabel(y_axis)
            ax.set_title("Model Trade-off Analysis", fontsize=12, fontweight='600', color='#1a365d')
            ax.grid(True, linestyle='--', alpha=0.6)
            st.pyplot(fig)
            st.caption("Models nearer to the lower-right region achieve better speed-accuracy balance.")
